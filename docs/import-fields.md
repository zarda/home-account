...
| Notes | the **Notes** button opens a textarea through the card's one editing machine — a row edits one field at a time, notes included — and it files on the way out, on blur | nothing | no | no |
...
| Row (removed) | **Remove** in the extras takes the row off the batch, on any row; focus lands on the neighbour's Remove or Add a row | everything keyed on its id — the card's editing state, the wizard's overrule, stamp and verdict | not for the removed row — nothing left to check — but yes for a survivor whose within-batch verdict named it, since that verdict is now about a row that has gone | no |
