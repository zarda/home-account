...
`translateReceiptImage` is its vision twin, and it inherits every one of those rules — no language list, the same `languageInstruction`, the same reproduction-not-rendition instruction, the same `{"translation", "sourceLanguage"}` answer read by the same mapper in the base. What it does not carry is any text at all: like every receipt prompt it takes no image, because the photo travels on the vision transport that attaches it, and the prompt is only the instruction that goes along with one. So it asks for a **transcription** rather than a translation of something already extracted — every line printed on the receipt, in the order it is printed, each translated, with amounts, dates and codes copied out exactly as printed. That distinction is the whole prompt: a model handed a receipt and told to translate it writes a summary of a shopping trip, which reads well and is not a receipt. Nothing is written back here either — the answer lives in the viewer and in a session cache, and the stored image is never touched ([translation-lens.md](translation-lens.md)).
